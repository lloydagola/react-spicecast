...
router.post("/test/:artist/:album", (req, res) => {
  ...
  upload(req, res, function (err) {
    ...
    req.files.map((file) =>
      readTrackTags(track_path + file.originalname)
        .then((song) => {
        ...
        .then((artist) => {
          if (!artist)
            return new contributingArtist({ title: _song.artist }).save();

          return artist;
        })
        .then((artist) => {
          if (!artist)
            throw new Error("artist doesn't exist and couldn't create one");
        ...
        .then((album) => {
          if (!album)
            throw new Error("album doesn't exist and couldn't create one");
        ...
        .then((track) => {
          if (!track)
            throw new Error("track doesn't exist and couldn't create one");
...
router.post("/:artist/:album", (req, res) => {
  if (!req.params.artist || !req.params.album)
    return res.status(400).send("Artist and album are both required!");
  ...
  const newAlbum = new album({
    ...
    contributingArtists: [newArtist._id],
  });
  newArtist
    .save()
